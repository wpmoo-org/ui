...
The server should emit the resolved `data-bs-theme` value on the owner. When a
stored browser preference is needed before visible content paints, the
canonical path is the external, parser-blocking, non-deferred
`theme-prepaint.js` script as the owner's first child. It applies
`stored > server > fallback`, resolves a stored `system` preference before it
writes the owner attribute, and finishes with `data-moo-prepaint="ready"` on
that owner. The default shared `moo:theme`/`moo:direction` keys are valid only
for a document with one top-level owner; independent nested or sibling owners
need explicit `data-moo-theme-key` and `data-moo-direction-key` values.

The prepaint asset is CSP-friendly because it is external and parser-blocking;
it does not require an inline theme script or inline theme style. It updates
`html[dir]` only for the complete-document owner, keeps theme state off
`html`/`body`, and uses an owner `dir` only for embedded subtree overrides.
